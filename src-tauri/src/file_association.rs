...
#[cfg(all(windows, feature = "windows-registry"))]
pub fn register_key_file_association() -> Result<(), String> {
    use std::env;
    ...
    let icon_path = {
        ...
        let prod_path = exe_dir.join("icons").join("cordia-key.ico");
        if prod_path.exists() {
            prod_path
        } else {
            // Try dev path (src-tauri/icons relative to exe)
            // In dev, exe is typically in target/debug/cordia.exe
            let dev_path = exe_dir
                .parent()
                .and_then(|p| p.parent())
                .map(|p| p.join("src-tauri").join("icons").join("cordia-key.ico"));
            
            if let Some(dev_path) = dev_path {
                if dev_path.exists() {
                    dev_path
                } else {
                    return Err(format!("cordia-key.ico not found. Tried: {} and {}. Please run: npm run generate-key-icon", 
                        prod_path.display(), dev_path.display()));
                }
            } else {
                return Err(format!("cordia-key.ico not found. Tried: {}. Please run: npm run generate-key-icon", 
                    prod_path.display()));
            }
        }
    };
    ...
    // Register .key file extension
    let (key_ext, _) = hkcu.create_subkey("Software\\Classes\\.key")
        .map_err(|e| format!("Failed to create .key key: {}", e))?;
    key_ext.set_value("", &"Cordia.Key")
        .map_err(|e| format!("Failed to set .key default value: {}", e))?;
    
    // Register Cordia.Key file type
    let (file_type, _) = hkcu.create_subkey("Software\\Classes\\Cordia.Key")
        .map_err(|e| format!("Failed to create Cordia.Key key: {}", e))?;
    file_type.set_value("", &"Cordia Key File")
